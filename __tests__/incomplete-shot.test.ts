...
(WASM_AVAILABLE ? describe : describe.skip).each(methods)(
    "Test Incomplete Shots with $name",
    (obj) => {
        const { method } = obj;
        ...
        describe.each(testPoints)(
            ...
            (distance, height, angleInDegrees) => {
                test("should get the same result with and without extra data", async () => {
                    ...
                    expect(distanceDifference).toBeLessThanOrEqual(
                        UNew.Foot(0.2).In(Distance.Meter)
                    );
                });
            }
        );
