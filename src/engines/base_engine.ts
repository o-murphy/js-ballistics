...
class BaseIntegrationEngine implements EngineInterface<BaseEngineConfig> {
    gravityVector: Vector;

    protected _config: BaseEngineConfig;
    ...
    protected _tProps: BaseEngineTrajectoryProps;
    ...
    protected _initTrajectory(shotInfo: Shot): void {
        ...
        this._tProps = {
            ...
            muzzleVelocity: shotInfo.ammo
                .getVelocityForTemp(shotInfo.atmo.powderTemp)
                .In(Velocity.FPS),
...
const calculateCurve = (dataPoints: DragTable): Curve => {
    ...
    let curve: Curve = [
        {
            a: 0,
            b: rate,
            c: dataPoints[0].CD - dataPoints[0].Mach * rate,
        },
    ];
    ...
    curve.push({
        a: 0,
        b: rate,
        c: dataPoints[numPoints - 1].CD - dataPoints[numPoints - 2].Mach * rate,
    });
    return curve;
};

// Extracts Mach values from a list of DragDataPoint objects.
const getOnlyMachData = (data: DragTable): number[] => {
