...
import { Angular, Distance, Unit } from "./unit";
...
class OutOfRangeError extends SolverRuntimeError {
    ...
    constructor(
        ...
        if (maxRange !== undefined) {
            message += ` exceeds maximum possible range ${maxRange.In(Unit.Foot)} feet`;
        }

        if (lookAngle !== undefined && lookAngle.rawValue) {
            message += ` with look-angle ${lookAngle.In(Unit.Radian)} rad`;
        }
